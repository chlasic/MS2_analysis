...
// Author: ChangHwan (Leo) Lee,     Aug 14, 2017
		...
		for (k = 1; k < fr+1; k++) {
			for (p = 1; p < 3; p++){
				...
				if (p==2){
			 	...
			 	run("MultiStackReg", "stack_1=["+title+"-"+ p*3-2 +".tif] action_1=Align file_1=c:\\CHL\\vector.txt stack_2=None action_2=Ignore file_2=[] transformation=[Rigid Body] save");
			 	if (p==1){
			 		run("Reverse");
			 	}
			 	if (p==2){
			...
			// align 3rd  channels using vectors gotten from 1st channel.
